...
export const testServerScenarios = {
  ...
  withSalesModule: () => createTestServer().enableModule('core').enableModule('sales'),
  withAllModules: () => createTestServer()
    .enableModule('core')
    .enableModule('sales')
...
export class IntegrationTestSuite {
  ...
  async testCRUDOperations(resourcePath: string, createData: any, updateData: any) {
    ...
    const resourceId = createdResource.id || createdResource[Object.keys(createdResource)[0]]?.id
    ...
    // Update
    const updateResponse = await this.server.put(`${resourcePath}/${resourceId}`, updateData)
    await this.server.expectSuccessResponse(updateResponse)
    
    // Delete
    const deleteResponse = await this.server.delete(`${resourcePath}/${resourceId}`)
    await this.server.expectSuccessResponse(deleteResponse)
    
    // Verify deletion
    const verifyResponse = await this.server.get(`${resourcePath}/${resourceId}`)
  ...
  async testValidation(resourcePath: string, invalidData: any, expectedErrors: string[]) {
