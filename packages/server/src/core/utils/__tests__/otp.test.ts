...
describe('OTPManager', () => {
  ...
  describe('storeOTP and verifyOTP', () => {
    it('should store and verify OTP successfully', () => {
      ...
      const { code } = OTPManager.storeOTP(email, 'email')

      const result = OTPManager.verifyOTP(email, 'email', code)
    ...
    it('should reject incorrect OTP', () => {
      ...
      OTPManager.storeOTP(email, 'email')
      const result = OTPManager.verifyOTP(email, 'email', '654321')

      expect(result.success).toBe(false)
    })

    it('should reject OTP for wrong email', () => {
      const { code } = OTPManager.storeOTP('test@example.com', 'email')
      const result = OTPManager.verifyOTP('other@example.com', 'email', code)
  ...
  describe('hasValidOTP', () => {
    it('should return true for valid stored OTP', () => {
      ...
      OTPManager.storeOTP(email, 'email')
      expect(OTPManager.hasValidOTP(email, 'email')).toBe(true)
    })

    it('should return false for non-existent email', () => {
      expect(OTPManager.hasValidOTP('nonexistent@example.com', 'email')).toBe(false)
    })
  })
  ...
  describe('getActiveCount', () => {
    it('should return correct count of active OTPs', () => {
      ...
      OTPManager.storeOTP('user1@example.com', 'email')
      expect(OTPManager.getActiveCount()).toBe(1)

      OTPManager.storeOTP('user2@example.com', 'email')
